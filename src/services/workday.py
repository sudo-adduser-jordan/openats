...
API silently loops back to the first page — so a naïve collector can never
...
_TIMETYPE_TO_EMPLOYMENT_TYPE: dict[str, EmploymentType] = {
    "full time": "FULL_TIME",
    "full_time": "FULL_TIME",
    "fulltime": "FULL_TIME",
    "part time": "PART_TIME",
...
@CollectorRegistry.register(ATSType.WORKDAY)
class WorkdayCollector(BaseCollector):
    """Workday collector — `company_slug` must be the full careers URL."""
    ...
    def fetch(self) -> list[Job]:
        slug = self.url or self.company_slug
        match = URL_PATTERN.match(slug.rstrip("/"))
        if not match:
    ...
    async def _exhaust_query(
        ...
        *,
        ...
        async with asyncio.TaskGroup() as tg:
            for v_id, _ in values:
                tg.create_task(child(v_id))
        ...
        *,
        ...
        async with asyncio.TaskGroup() as tg:
            for o in offsets:
                ...
                async def _do_fetch(o: int = o) -> None:
                    batches.append(await fetch_one(o))

                tg.create_task(_do_fetch())
    ...
    async def _request(
        ...
        *,
        ...
        raise CollectorError(
            f"Workday gave up after {MAX_RETRIES} retries at offset={offset}: {last_exc or f'HTTP {last_status}' or 'unknown'}"
        )
    ...
    def _parse_job(self, item: dict[str, Any], base_url: str, company: str) -> Job:
        ...
        for k in ("locations", "timeType", "jobFamilyGroup", "remoteType"):
            ...
            if v:
                raw[k] = v
        # Stash externalPath so the post-collect rollup-resolver can build
        # the detail URL without re-parsing the job url.
        if external_path:
            raw["externalPath"] = external_path

        return Job(
            url=as_url(f"{base_url}{external_path}" if external_path else base_url),
    ...
    *,
    ...
    for facet in facets:
        ...
        items: list[tuple[str, int]] = [
            (str(v.get("id")), int(v.get("count") or 0))
            for v in values
