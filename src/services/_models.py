...
These models are the canonical schema across every ATS collector and the
public dataset on storage.stapply.ai. Adding a field here means: the
dataset gets a new column, every collector must populate it (or leave
...
class ATSType(StrEnum):
    ...
    collector knows how to fetch its jobs and how the careers page is
    structured.
    """
    ...
    # collectors — each is the entire country's jobs api)
...
class WatchListEntry(BaseModel):
    """A company the user wants to watch."""

    model_config = ConfigDict(frozen=True)

    ats: str = Field(..., description="ATS type identifier")
    company_name: str
    company_slug: str
    notes: str = ""
    created_at: str | None = Field(None, description="ISO-8601 timestamp of when it was added")
...
class Job(BaseModel):
    ...
    Every collector produces ``Job`` instances; the public CSV/Parquet
    ...
    6. **Provider-specific overflow** (``raw``): a JSON dict captured
       at collect-time so we don't lose ATS-specific fields the
    ...
    ats_type: ATSType = Field(
        ...
        description=(
            ...
            "collector that produced the row and the format of "
            "``ats_id``."
        ),
    )
    ats_id: str | None = Field(
        ...
        description=(
            ...
            "row, and an error is logged so the broken collector is "
            "noticed."
        ),
    )
    ...
    country_iso: str | None = Field(
        ...
        description=(
            ...
            "``BR``, …). Set by the collector when the source ATS "
    ...
    is_remote: bool | None = Field(
        ...
        description=(
            ...
            "collector when the ATS exposes a flag. Otherwise inferred "
            "from the **title** by ``openats.enrichment.infer_is_remote`` "
    ...
    salary_min: float | None = Field(
        ...
        description=(
            ...
            "Either set directly by the collector from a structured ATS "
            "field, or derived from ``salary_summary`` via "
            "``openats.enrichment.parse_salary_range`` at publish time."
        ),
    )
    ...
    fetched_at: datetime | None = Field(
        default=None,
        description="When openats last saw this posting (UTC).",
    )
    language: str | None = Field(
        ...
        description=(
            ...
            "``ja``, …). Set by the collector when the source ATS exposes "
    ...
    def _populate_global_id(self) -> Self:
        ...
        identifier instead of failing the whole collect.
        """
        ...
        if normalized_id is None:
            log.error(
                ...
                "falling back to UUID. Source collector should be fixed.",
                self.ats_type.value,
                self.url,
                self.ats_id,
            )
